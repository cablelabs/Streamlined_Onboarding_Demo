...
* Starting the administrative OCF components
  * OBT (on AP or separate device that is already connected to the network)
  * Diplomat (on AP)
...
#### DHCP Named Pipe Caveat

Note that the named pipe used by the Diplomat for notification of a device
receiving an IP address may be incorrectly created by the DHCP leases notify
script as a plain text file instead of a named pipe. To ensure that the Diplomat
functions properly, it is recommended that it be restarted in the following way:

* Stop the currently running diplomat
* Delete the leases notification file (`/var/run/diplomat/leases`)
* Immediately restart the diplomat
...
* Observe Diplomat (option `41`)
  * Note that observing the Diplomat only needs to take place once. An initial
    response from the Diplomat will be presented to the OBT, and any subsequent
    notifications generated by the Diplomat will be sent to the OBT.
...
* Once all components are configured and started, the following devices should
  ...
  * AP/OCF administrative device:
    ...
    * DPP Diplomat
    * OBT (CLI or web-based; may be executed on a separate device depending on
      [requirements](../README.md#obt-and-vlans))
  * Client device(s):
